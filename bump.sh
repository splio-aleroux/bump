...
if [ -f VERSION ]; then
    ...
    V_BASE_LIST=(`echo $BASE_STRING | tr '.' ' '`)
    NV_MAJOR=${V_BASE_LIST[0]}
    NV_MINOR=${V_BASE_LIST[1]}
    ...
    if [ "$RESPONSE" = "y" ]; then
        ...
        echo "" >> CHANGELOG-0.1.md
        echo "" >> CHANGELOG-0.1.md
        echo -e "$ADJUSTMENTS_MSG"
        read
        echo -e "$PUSHING_MSG"
        git add VERSION CHANGELOG-0.1.md
        git commit -m "Add VERSION and CHANGELOG-0.1.md files, Bump version to v0.1.0."
